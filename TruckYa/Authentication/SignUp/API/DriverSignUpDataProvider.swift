...
import Foundation
import JSSAlertView
extension DriverSignupVC {
    internal func initiateSignUpSequence(name:String, email:String, password:String, phoneNumber:String) {
        SignUpAPI.shared.signUp(name: name, email: email, password: password, phoneNumber: phoneNumber, userType: .Driver) { (data, serviceError, error) in
            print("Entering Driver SignUp API")
            if let error = error {
                ...
                do {
                    ...
                    if let userData = result.data,
                        ...
                        print("Reached Driver User Details.")
                        print("Logging User Type at Driver => \(String(describing: userDetails.usertype))")
